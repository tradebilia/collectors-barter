...
| **Privacy-policy wording needs alignment with manual deletion workflow** | The policy mentions self-service deletion, while account deletion is currently an administrator-reviewed request. This is a disclosure/expectation problem, not evidence that a user cannot request deletion. | Update wording to describe the actual review workflow and expected completion process. |
...
| --- | --- | --- | --- |
| **Next — P2A** | Align the Privacy Policy account-deletion copy with the implemented reviewed-closure process. | The policy instructs a member to click “Delete Account,” confirm deletion, and expects removal within 30 days; Account Settings instead offers only **Request Account Deletion Review** and directs the member to support because active trades and records may need protection. This is a real disclosure mismatch. | Copy-only repair to the Privacy Policy and its regression test. Do not create a new deletion endpoint, delete records, promise a service-level timeframe, or alter account-closure policy without separate approval. |
...
> **Recommended next approval:** P2A first. It is a small, user-facing truthfulness correction with no database, payment, provider, or schedule action. P2B should be completed before any future proposal to activate live billing. The OAuth, administrator-refactor, and webhook-comment items can remain deliberately separated from payment or marketplace changes.
