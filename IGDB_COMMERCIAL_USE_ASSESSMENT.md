...
> **RAWG activation update — August 18, 2026:** The separate RAWG source is no longer key/terms-gated. After user commercial-use confirmation and secure key validation, it is a second manual, administrator-only Video Game factual-metadata source. RAWG reports its own global release year and explicitly flags a discrepancy with a supplied listing year; it does not silently replace listing metadata. See `RAWG_TEST_AI_ACTIVATION_2026-08-18.md` for details.
