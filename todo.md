...
- [ ] Restore the enabled daily shipment-reminder job’s delivery to the existing `/api/scheduled/tradeReminders` callback, verify one successful controlled run, and preserve job identity, notification policy, and idempotency.
- [ ] Configure a secure valid provider-token encryption key and add clear startup or integration guardrails so eBay, Facebook, and LinkedIn token persistence cannot fail silently, with no token-value disclosure or migration of provider records.
