...
- [ ] Repair the deployed daily shipment-reminder schedule only after checkpointing the corrected route, and add an administrator Operations Health card for database, schedule, and recent external API status without creating new schedules or notifications.
- [ ] Add an administrator Action Queue that consolidates existing pending approvals, unverified merchants, pending reports/flags, open/urgent tickets, disputed trades, and overdue trade milestones as counts with safe deep links.
- [ ] Add an Active Trade Lifecycle Monitor with read-only status, participant names, deadline, tracking, receipt, and dispute visibility; retain all existing participant authorization and trade actions.
- [ ] Add read-only launch-health and future-membership-readiness summaries without enabling Free Launch removal, payment enforcement, Stripe live mode, or member restrictions.
- [ ] Add a privacy-safe administrator audit trail using existing custom-TiDB capability where possible, with a minimal non-destructive schema addition only if required after read-only preflight.
- [ ] Add administrator-only, explicitly scoped CSV exports for operational listings, trades, member status, and support metrics with no payment details, credentials, or raw provider payloads.
