...
- [ ] Add narrowly scoped custom-TiDB uniqueness protections for usernames, one user profile per member, report IDs, and watchlist entries only after fresh read-only duplicate checks, a database backup/recovery review, and focused conflict-handling tests.
- [ ] Make Stripe sandbox Membership webhook processing retry-safe after a temporary failed event while retaining test-only operation, raw-body signature verification, terminal duplicate protection, Free Launch, and inactive payment enforcement.
- [ ] Bind PayPal payment verification to the locked proposal’s agreed cash amount, payer, payee, currency, and allowed trade state before any provider verification or payment-record write, with no sandbox/live charge change.
- [ ] Apply server-side public-contact throttling, server-owned non-urgent anonymous priority, and explicit anonymous ticket attribution without changing legitimate contact-form messaging or administrator access.
- [ ] Produce and execute a controlled dependency-upgrade plan that prioritizes the reported production security advisories, avoids bulk automatic fixes, and runs regression validation after each compatible update batch.
