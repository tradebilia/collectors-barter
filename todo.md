...
- [ ] BLOCKER: Verify the new session has restored the existing managed project checkout through the project restart procedure, verified required secrets and the intended external CUSTOM_DATABASE_URL, and passed the handoff acceptance checks in SESSION_HANDOFF_GUIDE.md.
- [ ] FRESH-SANDBOX BOOTSTRAP BLOCKER: A new task inside the Tradebilia Website project correctly receives project context and shared handoff files but may not receive the old `/home/ubuntu/tradebilia-platform` checkout. Use the platform-managed project restart action to restore the existing checkout; do not create a new web project or manually clone a replacement checkout. Validate this revised path in a fresh task.
