...
- [ ] MIGRATION DECISION: Manus Support confirmed no documented method to bind a fresh Project task to the existing live WebDev project. Replace the old continuity-handoff assumption with a controlled migration plan for a new WebDev project; do not start feature work until migration acceptance is complete.
...
- [x] MEDIA STRATEGY DECISION: Rich selected Option B, project-to-project storage cutover. Bring the new WebDev project online first with code, secure configuration, and the intended external database; defer all customer-media copy, database URL rewrites, and old-storage retirement until Rich explicitly approves the separate media-cutover phase.
...
- [ ] STORAGE EVALUATION: Compare independent object-storage providers that work cleanly with Manus server-side applications, then obtain Rich’s decision before implementing or migrating any customer media.
- [ ] FREE-TIER STORAGE REVIEW: Verify current free allowances and upgrade constraints for portable external object storage, then recommend whether a free tier is suitable for Tradebilia’s initial production media volume.
- [ ] R2 IMPACT ASSESSMENT: Map exact existing upload, image-rendering, and database media-path code to the minimal Cloudflare R2 changes; estimate phased migration effort and rollback safeguards without implementing anything.
- [ ] R2 SETUP GATE: Document the user-owned bucket, restricted API-token, CORS, custom delivery-domain, and secure credential-handoff steps before any R2 integration or customer-media migration begins.
