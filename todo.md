...
- [ ] Repair the reachable Manus OAuth sign-in state flow so every `getLoginUrl` caller establishes and validates a one-time state cookie using the existing live-safe cookie policy, with focused regression coverage.
