...
- [x] Diagnose the WebDev preview-panel sign-in behavior: it uses an isolated temporary-domain session, and sign-in works after establishing a fresh preview session without changing the working flow on `tradebilia.manus.space`.
- [x] Trace the divergent GitHub and WebDev histories and prepare an approval-gated plan to establish one durable synchronized baseline without overwriting newer GitHub work or database state.
- [x] Make external GitHub `main` the canonical Tradebilia code history, create recoverable references, reconcile WebDev by reviewed compatible content, validate without migrations or database writes, and record the paired commit/checkpoint baseline; membership code remains explicitly migration-gated for separate approval.
- [x] Build and validate the approved Tradebilia Membership, future Stripe billing, grace-period, unpaid-access, admin-monitoring, and Verification & Forwarding Service foundation in WebDev first; then replace only superseded GitHub membership work with the reviewed WebDev implementation. The non-destructive custom TiDB membership migration is approved; retain Free Launch and keep Stripe/payment enforcement inactive.
- [x] Complete the WebDev Free Launch membership foundation: create the approved non-destructive custom TiDB tables and defaults, model future monthly/annual terms and grace periods, prepare the verification-service records, add dormant item-detail access gating, add Account Settings and Admin Billing visibility, and validate without Stripe charges or enforcement.
- [x] Remove the vertical scrollbar arrows from the Profile page tab strip while retaining horizontal tab access on narrow screens.
