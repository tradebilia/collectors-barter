...
- [ ] Inventory all configured third-party credentials and map each provider to a no-write validation method.
- [ ] Validate eligible data, messaging, and video API credentials through read-only or authentication-only checks.
- [ ] Assess OAuth, payment, and social credentials without initiating login, payment, SMS, email, or user-facing actions.
- [ ] Document credential validation results, limitations, and follow-up requirements without exposing secret values.
- [ ] Inspect the Git repository state and define a credential-safe commit scope for completed Tradebilia work.
- [ ] Run validation checks and review staged changes for errors, secrets, generated files, and one-time utilities.
- [ ] Commit the approved clean project changes and push them to `tradebilia/collectors-barter`.
- [ ] Confirm the local branch and GitHub remote are synchronized after the push.
