...
- [ ] APPROVED NEW-SESSION SHARED-DATABASE WORKSPACE: Prepare the exact secure procedure for a new development session to clone GitHub and connect to the existing `CUSTOM_DATABASE_URL` through secret settings, preserving current accounts/items while prohibiting raw secret sharing, migrations, destructive scripts, new domains, and new deployments without further approval.
