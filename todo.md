...
**What Doesn't Work:**
- [ ] Search queries return "No results found" even when matching items exist in database
- [ ] Direct SQL queries confirm items exist (e.g., "Star Wars #1" with ID 90001)
- [ ] The issue appears to be with how Drizzle ORM's `or()` function is handling multiple LIKE conditions

**Debugging Notes:**
- The `or()` function may be returning undefined when combining multiple LIKE conditions
- Added check to filter out undefined values, but search still returns no results
- Tried multiple approaches: nested OR, raw SQL, CONCAT - all failed
- Database queries work correctly when executed directly
- Suggests the issue is in the Drizzle ORM query builder, not the database

**Next Steps to Fix:**
- [ ] Try using Drizzle's `sql` helper to build raw SQL search instead of using `or()`
- [ ] Implement a separate search endpoint that bypasses Drizzle's query builder
- [ ] Consider using MySQL FULLTEXT search with MATCH...AGAINST syntax
- [ ] Debug by logging the actual SQL being generated by Drizzle
- [ ] Check if the issue is related to how `and(...whereClauses)` handles undefined values
