...
- [ ] ADVERSARIAL AUDIT: Re-test every claim in all three handoff documents from direct project, GitHub, shared-file, deployment, database, secret, storage, and acceptance-command evidence. Label every claim proven, expected-but-unproven, or blocked/unknown; do not declare readiness until unsupported claims are removed or qualified.
- [ ] HANDOFF PUBLISH BUG: `HANDOFF_ADVERSARIAL_AUDIT.md` is present locally and in project shared files but was omitted from the GitHub patch because untracked files are not included by `git diff`. Add it through a clean credential-safe GitHub commit, then re-run alignment checks.
...
- [ ] SECURITY VERIFICATION: Confirm both handoff entry documents explicitly prohibit using `.project-config.json` as a handoff/secret source, add regression coverage, and verify the committed documents contain the warning before the next checkpoint.
