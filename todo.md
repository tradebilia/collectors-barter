...
- [ ] Enforce saved profile visibility, inventory-value, and contact-request preferences across public profile, member search, inquiry, and direct-message paths; keep full addresses server-only for distance calculation and retain owner/administrator access as appropriate.
- [ ] Restore the accepted Review stage after mutual trade acceptance, retain item locks until the existing mutually confirmed Shipping transition, and make server-derived PayPal verification reachable only in the intended accepted cash-trade stage with full lifecycle regression coverage.
