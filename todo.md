...
- [x] Create OAuth callback endpoint handler
- [x] Create EbayConnection UI component for Account Settings
- [x] Create EbayProfileBadge component for profile display
- [x] Add EbayConnection to Account Settings integrations tab
- [x] Display eBay verification badge on profiles
...
- [x] Fix edit button path in Inventory.tsx from `/inventory/{id}/edit` to `/inventory/edit/{id}`
- [x] Verify edit page loads with correct route `/inventory/edit/:listingId`
- [x] Verify form populates with existing listing data in edit mode
- [x] Test editing a listing and saving changes
- [x] Verify success message displays after save
- [x] Verify user is redirected to inventory page after save
- [x] Write and pass 8 vitest tests for edit functionality
- [x] Edit Listing functionality complete and tested end-to-end

## Category Page Layout Refactoring - Independent Element Positioning
...
## KNOWN ISSUE: Search functionality returns no results (NEEDS INVESTIGATION)

**Status:** Partially Fixed - Needs Further Investigation

**What Works:**
- [x] Search bar accepts input and navigates to search page
- [x] Search button added and functional
- [x] Condition dropdown fixed (changed "excellent" to "very_good")
- [x] Error display added to show actual errors
- [x] Search scope expanded to include title, description, and certificationCompany

**What Doesn't Work (FIXED):**
- [x] Search queries return "No results found" even when matching items exist in database
- [x] Direct SQL queries confirm items exist
- [x] The issue appeared to be with how Drizzle ORM's `or()` function is handling multiple LIKE conditions

**Root Cause (IDENTIFIED & FIXED - June 7, 2026):**
- [x] Search wasn't looking in the itemDetails JSON column where category-specific fields are stored
- [x] Updated search to include itemDetails JSON column with LIKE search
- [x] Updated category-specific field filters to use JSON_CONTAINS for accurate matching
- [x] Now searches across: title, description, certificationCompany, and itemDetails JSON
- [x] Category-specific filters (manufacturer, year, team, set, sport, etc.) now use JSON_CONTAINS

**Solution Implemented:**
- [x] Added itemDetails JSON column to listings table
- [x] Updated getMarketplaceFeed to search itemDetails with LIKE for keyword search
- [x] Updated category-specific field filters to use JSON_CONTAINS for exact matching
- [x] All filter types now work correctly with the new itemDetails structure


## BUG: Category page filters don't work

- [x] Filter inputs on category pages don't actually filter results
- [x] Typing in filter fields and clicking Search shows all items instead of filtered results
- [x] Example: Typing "Star Wars" in Title filter on Comics page still shows all items
...
## Fix ALL Conditional Fields Across All Categories
- [x] Change all 70 conditional fields to have their proper parent requirement (required/recommended/optional) instead of 'conditional'
- [x] Verify condition fields (Is Graded = No) render in Optional section
- [x] Verify grading fields (Is Graded = Yes) render in Optional section  
- [x] Verify description field renders in Optional section
