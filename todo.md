...
- [ ] Replace the two brittle exact-string mobile layout tests with behavior-focused responsive contracts that verify visible Account Settings controls and authenticated top-bar actions without constraining implementation details.
- [ ] Tighten the scheduled accepted-trade cleanup query so a receipt-confirmation record is deleted only when the parent proposal remains eligible for automated cancellation, and add a regression case for an advanced trade.
- [ ] Improve Sign Up and Trade Room accessibility with properly associated labels, accessible validation feedback, and a clear accessible label for the cash-dialog close control, preserving current desktop/mobile layout and trade behavior.
- [ ] Reduce the initial JavaScript payload with deliberate route/module splitting for infrequently used authenticated and administrator tools, then measure the resulting production build without removing active features.
- [ ] Minimize browser-persisted authentication data to the preview-only display fields needed for the embedded-session fallback, add expiry and validation, and retain reliable logout cleanup and live-domain authentication behavior.
