...
- [ ] Make scheduled receipt escalation conditionally update only still-eligible statuses and record a system escalation only after a successful state change, preventing a delayed job from overwriting a newer trade state or duplicating its event log.
- [ ] Add consistently bounded timeout handling, temporary-failure classification, and safe regression coverage to the approved Resend email, Twilio, and PCGS external-request paths without changing credentials, provider policy, or sending messages.
- [ ] Run required core configuration/startup validation before Express begins serving requests, while distinguishing optional provider integrations and preserving custom-TiDB precedence and preview-safe authentication behavior.
