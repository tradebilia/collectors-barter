...
- [ ] CLONE-ONLY CONTINUATION: Prepare copy-ready instructions for a new session to clone `tradebilia/collectors-barter` into a separate development workspace, run local validation, and avoid any current-project database, storage, domain, or deployment changes until Rich explicitly approves a later migration.
