...
const accessKeyId = process.env.R2_STATIC_ACCESS_KEY_ID;
const secretAccessKey = process.env.R2_STATIC_SECRET_ACCESS_KEY;
const endpoint = process.env.R2_STATIC_ENDPOINT?.replace(/\/$/, '');
const bucket = 'tradebilia-static';
...
describe.runIf(hasCredentials)('Cloudflare R2 static credentials', () => {
  it('lists at most one static object without writing, deleting, or changing any asset reference', async () => {
    const now = new Date();
...
describe.skipIf(hasCredentials)('Cloudflare R2 static credentials', () => {
