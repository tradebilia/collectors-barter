...
describe("Coming Soon experience", () => {
  it("uses the supplied final transparent logo and has an opt-in email form", () => {
    expect(pageSource).toContain('/manus-storage/tradebilia-final-transparent_b18f659a.svg');
    expect(pageSource).toContain('alt="Tradebilia"');
    expect(pageSource).toContain('tradebilia-coming-soon-dense-category-workbench_c64ac671.png');
    expect(pageSource).toContain('className="mx-auto flex h-20 w-full max-w-[25rem] items-center justify-center');
