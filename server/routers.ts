...
function buildAutomaticHighValueScenePrompt(input: {
  ...
  const subject = [
    ...
    imageReferences ? `References extracted from the actual listing image: ${imageReferences}. Make these associated visual cues unmistakable in the environment.` : "",
  ].filter(Boolean).join(" ");

  return [
    "Create one cinematic, photorealistic 16:9 collector-background scene for a Tradebilia social listing.",
    subject,
    "The attached listing image is a visual reference only for associated public context. Do not redraw, recreate, or depict the collectible itself anywhere in the generated background: no card, slab, package, figure, exact product, or duplicate item. Instead, build 2–4 unmistakable item-associated environmental elements from the image and public metadata, such as the player’s team colors, sport equipment, venue, era, franchise motifs, materials, or display objects. For a Wayne Gretzky sports card, this can mean Edmonton/Oilers color cues, hockey rink lighting, sticks, skates, gloves, puck, and vintage hockey-card collector context—but never another card or a fake card.",
    "Composition is mandatory: create a visibly detailed collector environment across the whole 16:9 canvas. Reserve the entire upper 50% of the canvas as a strict banner-safe band plus a visible buffer. Keep the lower-left item zone visually calm and free of large objects because the renderer will place the exact listing image there once. Keep the center-right information lane low-contrast and free of featured props for title, facts, and value. Contextual objects may occupy the far edges and background only; never place an item-like duplicate or source-image recreation anywhere. Avoid broad black voids, broad dark gradients, empty studio backdrops, or an image where the scene is visible only on the right.",
