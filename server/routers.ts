...
function buildAutomaticHighValueScenePrompt(input: {
  ...
  return [
    ...
    subject,
    "The attached listing image is a visual reference for one complete, faithful generated representation of the exact collectible. Preserve the item’s full silhouette, orientation, proportions, border, slab or holder, and all visible non-sensitive visual structure; do not omit the top, bottom, corners, or major parts. For a sports card, generate the complete card or slab as the single left-side hero, using the player, team, sport, colors, equipment, venue, and era as supporting cues. Add 2–4 secondary environmental elements tied to the item, but never create a second copy of the collectible. Do not place the actual source photo on the right, and do not create a duplicate card, package, figure, or product elsewhere. The generated scene must not add unrelated people, readable text, watermarks, or invented labels.",
    "Composition is mandatory: create a visibly detailed collector environment across the whole 16:9 canvas. Reserve the entire upper 50% of the canvas as a strict banner-safe band plus a visible buffer. The complete generated hero must be entirely below that band, fully inside the lower-left zone at approximately x=6%–34% and y=55%–94%, with every edge visible and a clear gap above it. Never crop, clip, or let the banner, logo, or any text overlap the hero. Keep the center-right information lane low-contrast and free of featured props for title, facts, and value. The right side may contain only contextual background detail and lighting, never another item-like duplicate. Avoid broad black voids, broad dark gradients, empty studio backdrops, or an image where the scene is visible only on the right.",
          ...
          .map((category) => {
            ...
            if (sample) {
              ...
              return {
                ...
                visualHints: getTradeAlertVisualHints({ category, itemType: sample.itemType, title: sample.title, itemDetails: sample.itemDetails }),
