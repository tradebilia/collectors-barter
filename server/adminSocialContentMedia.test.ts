...
describe("admin social-content media upload contract", () => {
  ...
  it("generates automatic high-value scenes only from bounded public display metadata", () => {
    ...
    expect(routerSource).toContain("attached listing image is a visual reference for one complete, faithful generated representation");
    ...
    expect(routerSource).toContain("Add 2–4 secondary environmental elements tied to the item");
    ...
    expect(routerSource).toContain("one complete, faithful generated representation of the exact collectible");
    ...
    expect(routerSource).toContain("The right side may contain only contextual background detail and lighting");
