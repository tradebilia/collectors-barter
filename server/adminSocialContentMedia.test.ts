...
describe("admin social-content media upload contract", () => {
  ...
  it("generates automatic high-value scenes only from bounded public display metadata", () => {
    ...
    expect(routerSource).toContain("attached listing image is a visual reference only for associated public context");
    ...
    expect(routerSource).toContain("build 2–4 unmistakable item-associated environmental elements");
    ...
    expect(routerSource).toContain("Do not redraw, recreate, or depict the collectible itself anywhere in the generated background");
    expect(routerSource).toContain("strict banner-safe band");
    expect(routerSource).toContain("Keep the lower-left item zone visually calm and free of large objects");
    expect(routerSource).toContain("upper 50% of the canvas");
    expect(routerSource).toContain("Contextual objects may occupy the far edges and background only");
