...
describe("Profile Integrations provider logos", () => {
  it("uses each supplied provider logo in the Integration tab", () => {
    expect(accountSettings).toContain("/manus-storage/Paypal_25ebc114.png");
    expect(accountSettings).toContain("/manus-storage/WhatNot_ab669ac9.png");
    expect(accountSettings).toContain("/manus-storage/Facebooklogo_0c02c2d1.png");
    expect(accountSettings).toContain("/manus-storage/LinkedIn_df1e2c1e.webp");
    expect(accountSettings).toContain("/manus-storage/Ebaylogo_12a10426.png");
  });

  it("uses the supplied logos in the primary connection cards", () => {
    expect(ebayConnection).toContain("/manus-storage/Ebaylogo_12a10426.png");
    expect(facebookConnection).toContain("/manus-storage/Facebooklogo_0c02c2d1.png");
    expect(linkedInConnection).toContain("/manus-storage/LinkedIn_df1e2c1e.webp");
  });
});
