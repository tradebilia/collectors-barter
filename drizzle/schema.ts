...
export const deletedAccounts = mysqlTable("deletedAccounts", {
	...
	reason: text(),
	deletedAt: timestamp({ mode: 'date' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
},
(table) => [
...
export const ebayFeedbackHistory = mysqlTable("ebayFeedbackHistory", {
	...
	feedbackDate: timestamp({ mode: 'date' }).notNull(),
	createdAt: timestamp({ mode: 'date' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
},
(table) => [
...
export const lowFeedbackFlags = mysqlTable("lowFeedbackFlags", {
	...
	adminNotes: text(),
	flaggedAt: timestamp({ mode: 'date' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
...
export const referralRequests = mysqlTable("referralRequests", {
	...
	emailSentAt: timestamp({ mode: 'date' }),
...
export const tradeProposals = mysqlTable("tradeProposals", {
	...
	respondedAt: timestamp({ mode: 'date' }),
...
export const users = mysqlTable("users", {
	...
	lastActivityAt: timestamp({ mode: 'date' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
	...
	ebayMemberSince: timestamp({ mode: 'date' }),
	ebayConnectedAt: timestamp({ mode: 'date' }),
	ebayAccessToken: text(),
	ebayRefreshToken: text(),
	ebayTokenExpiresAt: timestamp({ mode: 'date' }),
},
(table) => [
