...
export const tradeActivityLog = mysqlTable("tradeActivityLog", {
	...
	eventType: mysqlEnum(['trade_created','partner_joined','item_added','item_removed','cash_added','cash_removed','proposal_sent','proposal_accepted','proposal_declined','trade_cancelled','tracking_submitted','items_received','trade_completed','payment_step_started','payment_verification_started','payment_verified','payment_verification_failed','cash_payment_method_selected','cash_payment_marked_sent','cash_payment_received_confirmed','cash_payment_dispute_opened','cash_payment_terms_reset']).notNull(),
	details: text(),
	createdAt: timestamp({ mode: 'string' }).default('CURRENT_TIMESTAMP').notNull(),
},
(table) => [
...
export const tradeProposals = mysqlTable("tradeProposals", {
	...
	recipientId: int().notNull().references(() => users.id),
	requestedListingId: int().references(() => listings.id),
	note: text(),
...
export const userProfiles = mysqlTable("userProfiles", {
	...
	businessPhone: varchar({ length: 40 }),
	businessEmail: varchar({ length: 320 }),
	businessWebsite: varchar({ length: 512 }),
...
export const users = mysqlTable("users", {
	...
	// Private direct-payment identifiers. These are never public-profile fields.
	venmoUsername: varchar({ length: 80 }),
	cashAppCashtag: varchar({ length: 80 }),
	zelleEmail: varchar({ length: 320 }),
	zellePhone: varchar({ length: 32 }),
	externalPaymentMethodsUpdatedAt: timestamp({ mode: 'string' }),
...
export const supportTickets = mysqlTable("supportTickets", {
	...
	adminNotes: text(),
	assignedAdminId: int().references(() => users.id),
	resolvedAt: timestamp({ mode: 'string' }),
	createdAt: timestamp({ mode: 'string' }).default('CURRENT_TIMESTAMP').notNull(),
	updatedAt: timestamp({ mode: 'string' }).defaultNow().onUpdateNow().notNull(),
},
(table) => [
	index("supportTickets_userId_idx").on(table.userId),
	index("supportTickets_status_idx").on(table.status),
	index("supportTickets_createdAt_idx").on(table.createdAt),
]);

export const supportTicketReplies = mysqlTable("supportTicketReplies", {
	id: int().autoincrement().notNull().primaryKey(),
	ticketId: int().notNull().references(() => supportTickets.id, { onDelete: 'cascade' }),
	senderId: int().notNull().references(() => users.id),
	message: text().notNull(),
	isAdminReply: tinyint().default(0).notNull(),
	createdAt: timestamp({ mode: 'string' }).default('CURRENT_TIMESTAMP').notNull(),
},
(table) => [
	index("supportTicketReplies_ticket_idx").on(table.ticketId),
	index("supportTicketReplies_sender_idx").on(table.senderId),
]);

export const flaggedContent = mysqlTable("flaggedContent", {
	id: int().autoincrement().notNull().primaryKey(),
	contentType: mysqlEnum(['listing','user','trade']).notNull(),
	contentId: int().notNull(),
	flaggedByUserId: int().notNull().references(() => users.id),
	reason: varchar({ length: 100 }).notNull(),
	description: text(),
	status: mysqlEnum(['pending','reviewed','dismissed','actioned']).default('pending').notNull(),
	adminNotes: text(),
	reviewedByAdminId: int().references(() => users.id),
	reviewedAt: timestamp({ mode: 'string' }),
	createdAt: timestamp({ mode: 'string' }).default('CURRENT_TIMESTAMP').notNull(),
},
(table) => [
	index("flaggedContent_contentType_idx").on(table.contentType),
	index("flaggedContent_status_idx").on(table.status),
	index("flaggedContent_createdAt_idx").on(table.createdAt),
]);

// ─── Direct Messages (DB-backed) ────────────────────────────────────────────
export const directMessageThreads = mysqlTable("directMessageThreads", {
	id: int().autoincrement().notNull().primaryKey(),
...
export const directMessages = mysqlTable("directMessages", {
	id: int().autoincrement().notNull().primaryKey(),
	threadId: int().notNull().references(() => directMessageThreads.id, { onDelete: 'cascade' }),
	senderId: int().notNull().references(() => users.id),
	subject: varchar({ length: 255 }),
	body: text().notNull(),
	isReadByRecipient: tinyint().default(0).notNull(),
	createdAt: timestamp({ mode: 'string' }).default('CURRENT_TIMESTAMP').notNull(),
},
(table) => [
	index("directMessages_thread_idx").on(table.threadId),
	index("directMessages_sender_idx").on(table.senderId),
...
export const tradePayments = mysqlTable("tradePayments", {
	...
	paymentIdentifier: varchar({ length: 320 }),
	paymentMethodSelectedAt: timestamp({ mode: 'string' }),
	transactionId: varchar({ length: 255 }),
	status: mysqlEnum(['pending','method_selected','sent','received','disputed','submitted','verified','failed']).default('pending').notNull(),
	verificationResult: text(),
	verifiedAt: timestamp({ mode: 'string' }),
	sentAt: timestamp({ mode: 'string' }),
	receivedAt: timestamp({ mode: 'string' }),
	disputeOpenedAt: timestamp({ mode: 'string' }),
	disputeOpenedBy: int(),
	disputeReason: varchar({ length: 500 }),
	createdAt: timestamp({ mode: 'string' }).default('CURRENT_TIMESTAMP').notNull(),
	updatedAt: timestamp({ mode: 'string' }).defaultNow().onUpdateNow().notNull(),
},
(table) => [
	index("tradePayments_proposalId_idx").on(table.proposalId),
