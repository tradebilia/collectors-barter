...
After the final direction-helper checkpoint propagated, the standard public application bundle was verified to contain the completed-trade preview labels **Items actually swapped**, **You received**, and **You sent**. The Trade Hub remains intentionally authenticated, so its live item data cannot be opened without a member session; the published bundle confirmation, direct direction tests, server data-contract regression, and preserved Trade Showcase ownership model verify the completed-exchange presentation path.
