...
describe("native Social graphic exporter", () => {
  ...
  it("routes high-value listings through the cinematic listing composition with a phrase footer", () => {
    ...
    expect(exporterSource).toContain("listingBannerBottom + 14 * scale");
  ...
  it("protects the left item zone and keeps tall listing details on the right", () => {
    ...
    expect(exporterSource).toContain("const detailX = 704 * scale");
    ...
    expect(exporterSource).toContain("drawFacts(context, promotion?.facts ?? [], detailX, detailY, detailWidth, scale, 50)");
    ...
    expect(highValueRenderer).toContain('"right"');
  ...
  it("uses a textured brush-stroke Trade Alert and compact circular exchange mark", () => {
    ...
    expect(TRADE_ALERT_BRUSH_IMAGE_URL).toContain("TradeAlertRichV2_097ffc75.webp");
