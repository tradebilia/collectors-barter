...
export function buildTradeReceiptLines(receipt: TradeReceiptInput): string[] {
  ...
  const sideLines = (label: string, side: TradeReceiptInput["mySide"]) => [
    label, `Participant: ${side.contactName || side.name}`,
  ...
  return ["TRADEBILIA TRADE RECEIPT", `Trade reference: ${receipt.tradeReference}`, `Status: ${receipt.status}`, `Created: ${formatDate(receipt.createdAt)}`, `Accepted: ${formatDate(receipt.acceptedAt)}`, `Shipping deadline: ${formatDate(receipt.shippingDeadline)}`, "", ...sideLines("YOUR SIDE", receipt.mySide), "", ...sideLines("TRADE PARTNER SIDE", receipt.theirSide), "", "This receipt records the Trade Room terms and shipment information shown when it was downloaded."];
}
export function downloadTradeReceipt(receipt: TradeReceiptInput): void {
  const document = new jsPDF({ unit: "pt", format: "letter" });
  let y = 54;
  for (const line of buildTradeReceiptLines(receipt)) {
    const wrapped = document.splitTextToSize(line, 500) as string[];
    if (y + wrapped.length * 16 > 740) { document.addPage(); y = 54; }
    ...
    document.text(wrapped, 54, y); y += wrapped.length * 16 + 5;
  }
