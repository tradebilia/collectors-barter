...
export function RecentTradesCarousel({ trades, isLoading = false }: { trades: RecentTrade[]; isLoading?: boolean }) {
  ...
  return (
    <section aria-labelledby="recent-trades-heading" className="mx-4 mb-6 py-3 sm:mx-0 lg:mx-8">
      ...
      {isLoading ? <div className="mt-4 h-48 animate-pulse rounded-2xl bg-white/80" aria-label="Loading recent trades" /> : !trade || !exchange ? <div className="mt-4 rounded-2xl border border-dashed border-violet-200 bg-white/80 p-8 text-center text-sm text-slate-600">Completed exchanges will appear here as collectors confirm their trades.</div> : <article key={trade.id} className={`mt-4 rounded-2xl border border-violet-100 bg-white p-5 shadow-sm transition-opacity duration-300 motion-reduce:transition-none sm:p-6 ${isFading ? "opacity-0" : "opacity-100"}`} aria-live="off">
        <div className="grid gap-5 lg:grid-cols-[1fr_auto_1fr] lg:items-center">
          <div className="rounded-2xl bg-violet-50/75 p-4 lg:flex lg:items-center lg:justify-end lg:gap-5"><p className="mb-3 text-center text-base font-bold text-violet-950 lg:mb-0 lg:shrink-0">{exchange.left.member.displayName || "Member"} traded</p><TradeItemList items={exchange.left.items} alignment="end" /></div>
          <div className="flex justify-center"><ArrowLeftRight className="h-10 w-10 text-violet-600" strokeWidth={2.4} aria-label="Completed exchange" /></div>
          <div className="rounded-2xl bg-sky-50/80 p-4 lg:flex lg:items-center lg:justify-start lg:gap-5"><TradeItemList items={exchange.right.items} alignment="start" /><p className="mt-3 text-center text-base font-bold text-sky-950 lg:mt-0 lg:shrink-0">{exchange.right.member.displayName || "Member"} traded</p></div>
        </div>
      </article>}
    </section>
  );
