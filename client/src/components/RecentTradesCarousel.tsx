...
export function RecentTradesCarousel({ trades, isLoading = false }: { trades: RecentTrade[]; isLoading?: boolean }) {
  ...
  return (
    <section aria-labelledby="recent-trades-heading" className="mx-2 mb-6 py-3 sm:mx-0 lg:mx-3">
      ...
      {isLoading ? <div className="mt-4 h-40 animate-pulse rounded-2xl bg-white/80" aria-label="Loading recent trades" /> : !trade || !exchange ? <div className="mt-4 h-[68rem] rounded-2xl border border-dashed border-violet-200 bg-white/80 p-8 text-center text-sm text-slate-600 md:h-[31rem] lg:h-[33rem]">Completed exchanges will appear here as collectors confirm their trades.</div> : <div className="relative mt-4 h-[68rem] overflow-hidden px-0 sm:px-2 md:h-[31rem] lg:h-[33rem] lg:px-3">
