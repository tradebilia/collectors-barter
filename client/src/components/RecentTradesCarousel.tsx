...
export function RecentTradesCarousel({ trades, isLoading = false }: { trades: RecentTrade[]; isLoading?: boolean }) {
  ...
  return (
    <section aria-labelledby="recent-trades-heading" className="mx-4 mb-6 rounded-3xl border border-violet-200 bg-gradient-to-br from-violet-50 via-white to-sky-50 p-4 shadow-[0_16px_36px_rgba(76,29,149,0.10)] sm:mx-0 sm:p-6 lg:mx-8">
      <div className="flex flex-col gap-1 text-center">
        <div className="mx-auto flex items-center gap-2 text-violet-700"><CheckCircle2 className="h-5 w-5" aria-hidden="true" /><span className="text-xs font-bold uppercase tracking-[0.16em]">Tradebilia exchange activity</span></div>
        <h2 id="recent-trades-heading" className="font-serif text-3xl font-medium tracking-[-0.035em] text-[#2d241e] sm:text-4xl">Recent Trades</h2>
        <p className="text-sm text-slate-600">Recently completed exchanges between collectors.</p>
      </div>

      {isLoading ? <div className="mt-5 h-44 animate-pulse rounded-2xl bg-white/80" aria-label="Loading recent trades" /> : !trade || !exchange ? <div className="mt-5 rounded-2xl border border-dashed border-violet-200 bg-white/80 p-8 text-center text-sm text-slate-600">Completed exchanges will appear here as collectors confirm their trades.</div> : <article key={trade.id} className={`mt-5 rounded-2xl border border-white bg-white/95 p-4 shadow-sm transition-opacity duration-300 motion-reduce:transition-none sm:p-5 ${isFading ? "opacity-0" : "opacity-100"}`} aria-live="off">
        <div className="flex flex-col gap-1 border-b border-slate-100 pb-3 text-center sm:flex-row sm:items-center sm:justify-between sm:text-left"><p className="text-xs font-bold uppercase tracking-[0.14em] text-violet-700">Completed trade</p><p className="text-xs font-medium text-slate-500">{formatCompletedDate(trade.completedAt)}</p></div>
        <div className="mt-4 grid gap-4 lg:grid-cols-[1fr_auto_1fr] lg:items-center">
          <div className="rounded-2xl bg-violet-50/75 p-3"><p className="mb-3 text-center text-sm font-bold text-violet-950 sm:text-left">{exchange.left.member.displayName || "Member"} traded</p><TradeItemList items={exchange.left.items} /></div>
          <div className="flex justify-center"><ArrowLeftRight className="h-9 w-9 text-violet-600" strokeWidth={2.4} aria-label="Completed exchange" /></div>
          <div className="rounded-2xl bg-sky-50/80 p-3"><p className="mb-3 text-center text-sm font-bold text-sky-950 sm:text-left">{exchange.right.member.displayName || "Member"} traded</p><TradeItemList items={exchange.right.items} /></div>
        </div>
      </article>}
    </section>
  );
