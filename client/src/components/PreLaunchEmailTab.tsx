...
export function PreLaunchEmailTab() {
  ...
  return (
    <div className="space-y-5">
      ...
      <Dialog open={previewOpen} onOpenChange={setPreviewOpen}><DialogContent className="max-w-2xl"><DialogHeader><DialogTitle>Email preview</DialogTitle><DialogDescription>Preview only. This does not create or send an email.</DialogDescription></DialogHeader><div className="overflow-hidden rounded-xl border border-[#ded6c8] bg-[#f7f4ee]"><div className="bg-[#0a0d22] bg-cover bg-center bg-no-repeat p-6 text-center" style={{ backgroundImage: "url(https://assets.tradebilia.com/Background_23084d14.jpg)" }}><img src="https://tradebilia.manus.space/manus-storage/tradebilia_email_spaced_dfb4be7e.svg" alt="Tradebilia" className="mx-auto h-auto w-full max-w-md" /></div><div className="space-y-4 p-7 text-slate-800"><h2 className="text-xl font-semibold">{subject || DEFAULT_SUBJECT}</h2><p className="whitespace-pre-wrap text-sm leading-7">{formattedMessage}</p><p className="border-t border-[#ded6c8] pt-4 text-xs text-slate-500">You are receiving this because you opted in for Tradebilia pre-launch updates. <span className="font-semibold text-violet-700 underline underline-offset-2">Unsubscribe from pre-launch updates</span></p><a href="https://tradebilia.manus.space" target="_blank" rel="noreferrer" className="text-sm font-semibold text-violet-800 underline underline-offset-2">Visit Tradebilia</a></div></div></DialogContent></Dialog>
