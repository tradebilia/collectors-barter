...
export function SocialContentManagerTab() {
  ...
  const hasGeneratedHighValueScene = Boolean(selectedDraft?.promotion?.generatedBackgroundUrl?.startsWith("/manus-storage/") && selectedDraft.promotion.generatedBackgroundVersion === 11);
  ...
  useEffect(() => {
    ...
    if (selectedDraft.promotion.generatedBackgroundUrl?.startsWith("/manus-storage/") && selectedDraft.promotion.generatedBackgroundVersion === 11) return;
    ...
    generateHighValueListingScene.mutateAsync({
      ...
      setDrafts((current) => current.map((draft) => draft.id !== selectedDraft.id || !draft.promotion
        ? draft
        : { ...draft, promotion: { ...draft.promotion, generatedBackgroundUrl: url, generatedBackgroundVersion: 11 }, updatedAt: new Date().toISOString() }));
    }).catch(() => {
