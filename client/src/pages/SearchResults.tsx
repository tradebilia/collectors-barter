...
export function SearchResults() {
  ...
  return (
    <div className={`min-h-screen ${searchTheme.pageClassName}`}>
      ...
      <main className="flex min-h-[32rem] w-full flex-col gap-0 px-4 py-6 sm:px-6 lg:flex-row lg:items-start lg:px-0 lg:py-8 2xl:px-0">
            ...
            <>
              <div className={viewMode === "grid" ? "mt-6 grid grid-cols-2 gap-3 sm:grid-cols-2 md:grid-cols-6" : "mt-6 space-y-3"}>
                {listings.map(listing => (
                  <Card key={listing.id} className={`${viewMode === "list" ? "flex min-h-36 items-stretch" : "overflow-hidden"} rounded-md border border-gray-200 bg-white text-[#153746] shadow-sm transition hover:-translate-y-0.5 hover:shadow-md`}>
                    <Link href={`/listings/${listing.id}`} className={`${viewMode === "list" ? "aspect-[7/9] w-32 shrink-0" : "block aspect-[7/9]"} bg-white p-0`}><img src={resolveTradebiliaListingImage({ title: listing.title, category: listing.category, primaryPhotoUrl: listing.primaryPhotoUrl })} alt={listing.title} className="h-full w-full object-contain" /></Link>
