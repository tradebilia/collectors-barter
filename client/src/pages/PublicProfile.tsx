...
export default function PublicProfile() {
  ...
  const { data: followData, refetch: refetchFollow } = trpc.market.isFollowingUser.useQuery(
    { followingId: numericUserId },
    { enabled: numericUserId > 0 && !!currentUser && currentUser.id !== numericUserId }
  );
  ...
  const toggleFollowMutation = trpc.market.toggleFollowUser.useMutation({
    onSuccess: (data) => {
      refetchFollow();
      toast.success(data.following ? 'Trader saved to your favorites!' : 'Trader removed from favorites');
    },
    onError: (err) => {
    ...
    <div className="min-h-screen bg-[#f8fafc] pb-20">
      ...
      <section className="relative z-0 w-screen -mx-[calc((100vw-100%)/2)] overflow-hidden text-white" style={{
        ...
        <div className="container relative flex h-64 items-center justify-center py-0 sm:h-72 lg:h-80">
          <div className="flex w-full max-w-6xl items-center justify-center -ml-32">
            <img src="/manus-storage/Profile_97aeba0f.svg" alt="Profile" className="h-auto w-full" />
          </div>
        </div>
      </section>
      <CategoryBar />
            ...
            <div className="flex items-start gap-2 w-full md:w-auto shrink-0">
              {currentUser?.id !== user.id ? (
                <>
                  ...
                  <Button
                    variant="outline"
                    className={`rounded-xl border-slate-200 font-bold px-4 transition-colors ${
                      isFollowing ? 'text-red-500 border-red-200 bg-red-50 hover:bg-red-100' : 'text-slate-600 hover:text-red-500'
                    }`}
                    onClick={() => toggleFollowMutation.mutate({ followingId: numericUserId })}
                    disabled={toggleFollowMutation.isPending}
                    title={isFollowing ? 'Remove from favorites' : 'Save trader to favorites'}
                  >
              ...
              <Button
                variant="outline"
                className="rounded-xl border-slate-200 text-slate-600 font-bold px-4 hover:text-blue-500 transition-colors"
                title="Copy profile link"
                onClick={() => {
                  const url = `${window.location.origin}/profile/${numericUserId}`;
      ...
      {composeOpen && profileData && (
        <ComposeMessageModal
          ...
          recipient={{
            id: user.id,
            displayName: profile?.displayName || user.displayName || user.username || 'Collector',
