...
const formatTradeRoomPhone = (value: unknown) => {
  const suppliedPhone = String(value ?? '').trim();
  const digits = suppliedPhone.replace(/\D/g, '');

  // Display standard North American numbers consistently without changing
  // the stored contact value. Preserve other number formats verbatim.
          ...
          {(currentStage === 'accepted' || currentStage === 'shipping' || currentStage === 'shipped' || currentStage === 'review' || currentStage === 'completed') && (() => {
            ...
            return (
              <div className="flex flex-col gap-4 flex-1">
                ...
                <div className={`bg-gradient-to-r from-green-900/40 to-emerald-900/30 border border-green-500/40 rounded-xl shadow-[0_0_30px_rgba(34,197,94,0.15)] ${isFinalizeStage ? 'px-4 py-3' : 'p-5'}`}>
                ...
                {currentStage !== 'shipping' && <div className={`bg-[#16213e] border border-gray-600 rounded-xl shadow-xl ${isFinalizeStage ? 'p-4' : 'p-5'}`}>
                  ...
                  <div className={`flex flex-col lg:flex-row lg:items-stretch ${isFinalizeStage ? 'gap-3' : 'gap-6'}`}>
                    ...
                    <div className={`flex-1 space-y-2 ${myReviewHasSingleItem ? 'lg:min-w-0' : ''}`}>
                      ...
                      {myItems.map((item: any) => (
                        <div key={item.id} className={`flex gap-3 bg-[#0f0f1a] border border-gray-700 rounded-lg ${myReviewHasSingleItem ? (isFinalizeStage ? 'flex-col p-3 sm:flex-row sm:items-center' : 'flex-col p-4') : 'items-center p-2'}`}>
                          {item.photos?.[0]?.imageUrl
                            ? <img src={item.photos[0].imageUrl} className={`${myReviewHasSingleItem ? (isFinalizeStage ? 'h-32 w-full sm:w-32' : 'w-full h-72') : 'w-28 h-28'} object-contain rounded-lg bg-[#0a0a1a] shrink-0`} alt={item.title} />
                            : <div className={`${myReviewHasSingleItem ? (isFinalizeStage ? 'h-32 w-full sm:w-32' : 'w-full h-72') : 'w-28 h-28'} bg-[#0a0a1a] rounded-lg flex items-center justify-center text-gray-600 text-xs shrink-0`}>No image</div>
                          }
                          <div className="flex-1 min-w-0">
                            <p className={`text-white font-semibold leading-tight ${myReviewHasSingleItem ? (isFinalizeStage ? 'text-lg' : 'text-xl') : 'text-base'}`}>{item.title}</p>
                    ...
                    <div className={`flex-1 space-y-2 ${theirReviewHasSingleItem ? 'lg:min-w-0' : ''}`}>
                      ...
                      {theirItems.map((item: any) => (
                        <div key={item.id} className={`flex gap-3 bg-[#0f0f1a] border border-gray-700 rounded-lg ${theirReviewHasSingleItem ? (isFinalizeStage ? 'flex-col p-3 sm:flex-row sm:items-center' : 'flex-col p-4') : 'items-center p-2'}`}>
                          {item.photos?.[0]?.imageUrl
                            ? <img src={item.photos[0].imageUrl} className={`${theirReviewHasSingleItem ? (isFinalizeStage ? 'h-32 w-full sm:w-32' : 'w-full h-72') : 'w-28 h-28'} object-contain rounded-lg bg-[#0a0a1a] shrink-0`} alt={item.title} />
                            : <div className={`${theirReviewHasSingleItem ? (isFinalizeStage ? 'h-32 w-full sm:w-32' : 'w-full h-72') : 'w-28 h-28'} bg-[#0a0a1a] rounded-lg flex items-center justify-center text-gray-600 text-xs shrink-0`}>No image</div>
                          }
                          <div className="flex-1 min-w-0">
                            <p className={`text-white font-semibold leading-tight ${theirReviewHasSingleItem ? (isFinalizeStage ? 'text-lg' : 'text-xl') : 'text-sm'}`}>{item.title}</p>
                            <p className="text-gray-200 text-base font-mono mt-1">Ref # {String(item.id).padStart(5, '0')}</p>
