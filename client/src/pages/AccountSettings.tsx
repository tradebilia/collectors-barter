...
export default function AccountSettings() {
  ...
  const paypalIdentityQuery = trpc.market.getPayPalIdentity.useQuery(undefined, { enabled: isAuthenticated });
  const disconnectPayPalIdentityMutation = trpc.market.disconnectPayPalIdentity.useMutation({
    onSuccess: async () => {
      await paypalIdentityQuery.refetch();
      toast.success("PayPal connection removed.");
    },
    onError: (error) => toast.error(error.message),
  });
              ...
              <Card className="rounded-[1.5rem] border-slate-200 bg-white shadow-sm">
                ...
                <CardContent className="space-y-6">
                  ...
                  <div className="space-y-3">
                    ...
                    <LinkedInConnection />
                    <EtsyConnection />
                    <div className="rounded-xl border border-sky-100 bg-gradient-to-br from-white to-sky-50/60 p-4 shadow-sm sm:p-5">
                      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                        <div className="flex min-w-0 items-start gap-3">
                          <img src={externalPaymentMethodOptions[0].logo} alt="PayPal" className="h-16 w-16 rounded-xl object-contain" />
                          <div className="min-w-0">
                            <div className="flex flex-wrap items-center gap-2">
                              <h3 className="font-semibold text-slate-950">PayPal Account</h3>
                              <span className="rounded-full border border-sky-200 bg-sky-50 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-sky-700">Private connection</span>
                            </div>
                            <p className="mt-1 max-w-2xl text-sm leading-6 text-slate-600">Connect PayPal through its authorization page to confirm control of the account and retrieve only the approved profile claims. Sensitive details are not shown publicly.</p>
                          </div>
                        </div>
                        {paypalIdentityQuery.data ? (
                          <span className="shrink-0 rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-semibold text-emerald-700">Connected</span>
                        ) : null}
                      </div>
                      {paypalIdentityQuery.data ? (
                        <div className="mt-4 rounded-lg border border-slate-200 bg-white p-4">
                          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <div>
                              <p className="font-semibold text-slate-950">PayPal account connected</p>
                              <p className="text-xs text-slate-500">Connected {new Date(paypalIdentityQuery.data.connectedAt).toLocaleString()}</p>
                            </div>
                            <Button type="button" variant="ghost" size="sm" className="rounded-lg bg-red-600 px-3 py-2 text-white hover:bg-red-700" disabled={disconnectPayPalIdentityMutation.isPending} onClick={() => disconnectPayPalIdentityMutation.mutate()}>
                              Disconnect
                            </Button>
                          </div>
                          <div className="mt-3 flex flex-wrap gap-2 border-t border-slate-100 pt-3 text-xs text-slate-600">
                            {paypalIdentityQuery.data.verifiedAccount === true ? <span className="rounded-full bg-emerald-50 px-2.5 py-1 font-semibold text-emerald-700">PayPal account verified</span> : null}
                        ...
                        <div className="mt-4 flex flex-col gap-3 rounded-lg border border-slate-200 bg-white p-4 sm:flex-row sm:items-center sm:justify-between">
                          <p className="text-sm text-slate-600">You will authorize PayPal directly. Tradebilia does not receive your PayPal password.</p>
                          <Button type="button" className="rounded-lg bg-blue-600 text-white hover:bg-blue-700" onClick={() => window.location.assign("/api/paypal/start")}>Connect PayPal</Button>
                        </div>
                      )}
                    </div>
