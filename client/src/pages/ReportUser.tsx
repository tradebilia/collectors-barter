...
export default function ReportUser() {
  ...
  const contactEmailAccountId = useRef<number | null>(null);
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => { const files = Array.from(event.target.files || []); event.target.value = ""; void uploadEvidenceFiles(files); };
  const openEvidencePicker = () => { if (!isUploading) evidenceInputRef.current?.click(); };
  const handleEvidenceDrop = (event: React.DragEvent<HTMLDivElement>) => { event.preventDefault(); setIsEvidenceDropActive(false); void uploadEvidenceFiles(Array.from(event.dataTransfer.files)); };
  const handleEvidenceDropzoneKeyDown = (event: React.KeyboardEvent<HTMLDivElement>) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); openEvidencePicker(); } };
  ...
  return <div className="min-h-screen bg-[radial-gradient(circle_at_top,rgba(73,125,255,0.14),transparent_30%),linear-gradient(180deg,#050814_0%,#0b1220_32%,#111827_100%)] text-white"><TopBar hideSearch /><section className="relative z-0 w-screen -mx-[calc((100vw-100%)/2)] overflow-hidden border-b border-white/10 bg-[#00143A] text-white"><div className="absolute inset-0" style={{ backgroundImage: `url(${REPORT_USER_HERO_BACKGROUND_URL})`, backgroundSize: "cover", backgroundPosition: "center", backgroundRepeat: "no-repeat" }} /><div className="container relative flex h-64 items-center justify-center py-0 sm:h-72 lg:h-80"><img src="/manus-storage/ReportaMember_d6e803c4.svg" alt="Report a Member" className="h-auto w-full max-w-7xl object-contain px-4" /></div></section><CategoryBar /><main className="container py-8"><div className="mx-auto max-w-3xl"><Card className="border-white/10 bg-white/5 backdrop-blur"><CardHeader className="border-b border-white/10"><CardTitle className="flex items-center gap-2 text-white"><Flag className="h-5 w-5 text-red-400" />Report a Member</CardTitle><CardDescription className="text-white/60">Report a community safety concern. For a trade-specific item or shipment issue, use the Trade Room link so the case includes the correct trade context.</CardDescription><div className="mt-4"><Link href="/my-reports"><Button type="button" variant="outline" size="sm" className="border-blue-500 bg-blue-600 text-white hover:bg-blue-700"><FileText className="mr-2 h-4 w-4" />View My Reports</Button></Link></div></CardHeader><CardContent className="pt-6">
    {submittedReportId ? <div className="space-y-5 text-center"><CheckCircle2 className="mx-auto h-12 w-12 text-green-400" /><h2 className="text-xl font-semibold">Report submitted</h2><p className="text-white/70">Your private reference is <span className="font-mono text-blue-200">{submittedReportId}</span>. You can view its status in My Reports.</p><div className="flex justify-center gap-3"><Link href="/my-reports"><Button className="bg-blue-600 text-white hover:bg-blue-700">View My Reports</Button></Link><Button variant="outline" className="border-white/15 bg-transparent text-white" onClick={() => setSubmittedReportId(null)}>Submit another</Button></div></div> : <form onSubmit={submitReport} className="space-y-6">
      ...
      <div className="grid gap-5 md:grid-cols-2"><div className="space-y-2"><Label className={fieldLabelClass}>Member username</Label><div className="relative"><Input value={reportedMember} onChange={event => { setReportedMember(event.target.value); setResolvedUserId(null); }} onBlur={handleUsernameBlur} className="h-12 border-slate-200 bg-white pr-20 text-slate-900 placeholder:text-slate-500" placeholder="Username of member to report" />{isLookingUp && <span className="absolute right-3 top-3.5 text-xs text-white/60">Checking</span>}{resolvedUserId && !isLookingUp && <span className="absolute right-3 top-3.5 text-xs text-green-300">Verified</span>}</div>{usernameError && <p className="text-xs text-red-300">{usernameError}</p>}</div><div className="space-y-2"><Label className={fieldLabelClass}>Listing or trade reference</Label><Input value={listingReference} onChange={event => setListingReference(event.target.value)} className="h-12 border-slate-200 bg-white text-slate-900 placeholder:text-slate-500" placeholder="Optional listing ID, proposal ID, or URL" /></div></div>
      <div className="grid gap-5 md:grid-cols-[1fr_240px]"><div className="space-y-2"><Label className={fieldLabelClass}>Concern type</Label><Select value={concernType} onValueChange={value => setConcernType(value as typeof concernType)}><SelectTrigger className="h-12 border-slate-200 bg-white text-slate-900 placeholder:text-slate-500"><SelectValue placeholder="Choose the issue category" /></SelectTrigger><SelectContent>{concernTypes.map(type => <SelectItem key={type} value={type}>{type}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><Label className={fieldLabelClass}>Contact email</Label><Input value={contactEmail} onChange={event => { setContactEmail(event.target.value); setContactEmailEdited(true); }} className="h-12 border-slate-200 bg-white text-slate-900 placeholder:text-slate-500" placeholder="you@example.com" /></div></div>
      <div className="space-y-2"><Label className={fieldLabelClass}>What happened?</Label><Textarea value={details} onChange={event => setDetails(event.target.value)} className="min-h-40 border-slate-200 bg-white text-slate-900 placeholder:text-slate-500" placeholder="Describe the timeline, what was affected, and why you are concerned." /></div>
      <div className="space-y-2"><Label className={fieldLabelClass}>Evidence notes</Label><Textarea value={supportingNotes} onChange={event => setSupportingNotes(event.target.value)} className="min-h-28 border-slate-200 bg-white text-slate-900 placeholder:text-slate-500" placeholder="Explain what each attachment shows, or add relevant tracking, listing, or message details." /></div>
      <div className="space-y-3"><Label className={fieldLabelClass}>Evidence attachments</Label><div role="button" tabIndex={0} aria-describedby="report-evidence-help" aria-disabled={isUploading} onClick={openEvidencePicker} onKeyDown={handleEvidenceDropzoneKeyDown} onDragOver={event => { event.preventDefault(); if (!isUploading) setIsEvidenceDropActive(true); }} onDragLeave={() => setIsEvidenceDropActive(false)} onDrop={handleEvidenceDrop} className={`cursor-pointer rounded-[1.25rem] border-2 border-dashed p-6 text-center transition-colors ${isEvidenceDropActive ? "border-blue-300 bg-blue-400/15" : "border-white/20 bg-white/5"} ${isUploading ? "cursor-wait opacity-70" : "hover:border-blue-300/70 hover:bg-blue-400/10"}`}><input ref={evidenceInputRef} type="file" multiple onChange={handleFileUpload} className="hidden" accept="image/jpeg,image/png,image/webp,application/pdf,text/plain" /><Upload className="mx-auto h-6 w-6 text-white/70" /><p className="mt-2 text-sm font-medium text-white">Drop evidence here or click to browse</p><p id="report-evidence-help" className="mt-1 text-xs text-white/60">PNG, JPG, WEBP, PDF, or TXT · up to 10MB each · maximum 5 files</p></div>{isUploading && <p className="flex items-center gap-2 text-xs text-blue-200"><Loader2 className="h-3.5 w-3.5 animate-spin" />Uploading evidence securely…</p>}{attachments.map((file, index) => <div key={file.key} className="flex items-center justify-between rounded-lg bg-white/10 p-3 text-sm"><span className="flex min-w-0 items-center gap-2 truncate"><FileText className="h-4 w-4 shrink-0 text-blue-200" />{file.name}</span><Button type="button" size="icon" variant="ghost" className="h-7 w-7 text-white/70" onClick={() => setAttachments(current => current.filter((_, i) => i !== index))}><X className="h-4 w-4" /></Button></div>)}</div>
      <div className="flex flex-wrap items-center gap-3"><Button type="submit" className="rounded-full bg-white text-slate-950 hover:bg-white/90" disabled={reportMutation.isPending || isUploading}>{reportMutation.isPending ? "Submitting report…" : "Submit report"}</Button></div>
    </form>}
