...
export default function ReportUser() {
  ...
  return <div className="min-h-screen bg-[radial-gradient(circle_at_top,rgba(73,125,255,0.14),transparent_30%),linear-gradient(180deg,#050814_0%,#0b1220_32%,#111827_100%)] text-white"><TopBar hideSearch /><section className="relative z-0 w-screen -mx-[calc((100vw-100%)/2)] overflow-hidden border-b border-white/10 bg-[#00143A] text-white"><div className="absolute inset-0" style={{ backgroundImage: `url(${REPORT_USER_HERO_BACKGROUND_URL})`, backgroundSize: "cover", backgroundPosition: "center", backgroundRepeat: "no-repeat" }} /><div className="container relative flex h-64 items-center justify-center py-0 sm:h-72 lg:h-80"><img src="/manus-storage/ReportaMember_d6e803c4.svg" alt="Report a Member" className="h-auto w-full max-w-7xl object-contain px-4" /></div></section><CategoryBar /><main className="container py-8"><div className="mx-auto max-w-3xl"><Card className="border-white/10 bg-white/5 backdrop-blur"><CardHeader className="border-b border-white/10"><CardTitle className="flex items-center gap-2 text-white"><Flag className="h-5 w-5 text-red-400" />Report a Member</CardTitle><CardDescription className="text-white/60">Report a community safety concern. For a trade-specific item or shipment issue, use the Trade Room link so the case includes the correct trade context.</CardDescription><div className="mt-4"><Link href="/my-reports"><Button type="button" variant="outline" size="sm" className="border-blue-300/40 bg-blue-500/15 text-blue-100 hover:bg-blue-500/25"><FileText className="mr-2 h-4 w-4" />View My Reports</Button></Link></div></CardHeader><CardContent className="pt-6">
    {submittedReportId ? <div className="space-y-5 text-center"><CheckCircle2 className="mx-auto h-12 w-12 text-green-400" /><h2 className="text-xl font-semibold">Report submitted</h2><p className="text-white/70">Your private reference is <span className="font-mono text-blue-200">{submittedReportId}</span>. You can view its status in My Reports.</p><div className="flex justify-center gap-3"><Link href="/my-reports"><Button>View My Reports</Button></Link><Button variant="outline" className="border-white/15 bg-transparent text-white" onClick={() => setSubmittedReportId(null)}>Submit another</Button></div></div> : <form onSubmit={submitReport} className="space-y-6">
      ...
      <div className="grid gap-5 md:grid-cols-[1fr_240px]"><div className="space-y-2"><Label className={fieldLabelClass}>Concern type</Label><Select value={concernType} onValueChange={value => setConcernType(value as typeof concernType)}><SelectTrigger className="h-12 border-white/10 bg-white/5 text-white"><SelectValue placeholder="Choose the issue category" /></SelectTrigger><SelectContent>{concernTypes.map(type => <SelectItem key={type} value={type}>{type}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><Label className={fieldLabelClass}>Contact email</Label><Input value={contactEmail} onChange={event => { setContactEmail(event.target.value); setContactEmailEdited(true); }} className="h-12 border-white/10 bg-white/5 text-white" placeholder="you@example.com" /></div></div>
      <div className="space-y-2"><Label className={fieldLabelClass}>What happened?</Label><Textarea value={details} onChange={event => setDetails(event.target.value)} className="min-h-40 border-white/10 bg-white/5 text-white" placeholder="Describe the timeline, what was affected, and why you are concerned." /></div>
      <div className="space-y-2"><Label className={fieldLabelClass}>Evidence notes</Label><Textarea value={supportingNotes} onChange={event => setSupportingNotes(event.target.value)} className="min-h-28 border-white/10 bg-white/5 text-white" placeholder="Explain what each attachment shows, or add relevant tracking, listing, or message details." /></div>
